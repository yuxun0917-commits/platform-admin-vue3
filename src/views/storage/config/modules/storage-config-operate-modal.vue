...
import { fetchStorageConfigAdd, fetchStorageConfigEdit, fetchStorageConfigEnums } from '@/service/api';
...
function setFormFromRow() {
  if (props.row) {
    ...
    formModel.storageType = props.row.storageType;
    formModel.endpoint = props.row.endpoint;
    formModel.region = props.row.region;
    formModel.bucket = props.row.bucket;
    // accessKey / secretKey 后端脱敏返回，编辑时留空表示保持不变（后端保留原值）
    formModel.accessKey = '';
    formModel.secretKey = '';
    formModel.basePath = props.row.basePath;
    formModel.domain = props.row.domain;
    formModel.isHttps = props.row.isHttps;
